<template>
  <div class="baking">
    <v-container>
      <!-- Loading spinner -->
      <div class="d-flex justify-center mt-15" v-if="bakingPosts == ''">
        <v-progress-circular
        indeterminate
        color="primary"
        ></v-progress-circular>
      ...
      <transition name="posts-cards">
        <transition-group name="posts-cards" class="d-flex flex-wrap justify-center my-10 px-md-15" v-if="bakingPosts != ''">
          <div
          v-for="(post) in bakingPosts"
          :key="post.id"
...
export default {
  ...
  methods: {
    getWpPosts () {
      axios.get('https://ms-portfolio.eu/wp-json/wp/v2/posts?categories=56')
        ...
        .then(data => {
          if (data) {
            this.allBakingPosts = data
            this.bakingPosts = this.allBakingPosts.slice(0, this.postsToLoad)
          }
        })
        .catch(error => {
          console.log(error)
    ...
    loadMoreWpPosts () {
      window.onscroll = () => {
        if (this.allBakingPosts !== '' && window.location.pathname === '/baking') {
          const scrollTrigger = document.documentElement.scrollTop + window.innerHeight
          const bottomOfMain = document.getElementsByClassName('baking')[0].scrollHeight
  ...
  watch: {
    bakingPosts: function () {
      this.bakingPosts.forEach(post => {
